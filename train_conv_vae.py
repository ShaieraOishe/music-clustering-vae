...
from src.vae import ConvVAE, vae_loss

def train_conv_vae(model, dataloader, optimizer, device, epoch):
    """
    train convolutional vae for one epoch
    
    parameters:
    - model: conv-vae model
...
def extract_latent_features(model, dataloader, device):
    """
    extract latent features from trained conv-vae
    
    parameters:
    - model: trained conv-vae model
    - dataloader: data loader
    - device: cpu or cuda
...
def main():
    """
    main training script for convolutional vae
    """
    ...
    latent_dim = 16  # same as basic vae
    batch_size = 32
    learning_rate = 0.001
    num_epochs = 50
    ...
    # create model
    print("\ncreating convolutional vae model...")
    ...
    for epoch in range(1, num_epochs + 1):
        train_loss = train_conv_vae(model, dataloader, optimizer, device, epoch)
    ...
    model_path = os.path.join(model_dir, 'conv_vae_model.pth')
    ...
    latent_path = os.path.join(features_dir, 'conv_vae_latent_features.pkl')
